// Team roster data for MGMT 6100 Spring 2026
const SURVEY_CONFIG = {
  course: "MGMT 6100 Seminar in Human Resource Management (Spring 2026)",
  apiUrl: "https://script.google.com/macros/s/AKfycbwa2YzChXUPVawiyipWzAM_Q46HoFwKzIAJyVUjLbsqd2MBMqWF9OkTCMD4kEh2--Wv/exec",

  activities: [
    "Team meeting 1",
    "Team meeting 2",
    "Team meeting 3",
    "Team meeting 4",
    "Team meeting 5",
    "Team meeting 6",
    "Team meeting 7",
    "Team meeting 8",
    "Team meeting 9",
    "Team meeting 10",
    "Team meeting 11",
    "Team Presentation (Dates vary between teams)"
  ],

  teams: [
    {
      number: 1,
      members: ["Maria Arrieta Perez", "Laura Carrillo", "Eduardo Avalos"]
    },
    {
      number: 2,
      members: ["Bhavika Damani", "Madison Guerrero", "Angel Avina"]
    },
    {
      number: 3,
      members: ["Yvette Jackson", "Prabhjot Kaur", "Juan Chavez"]
    },
    {
      number: 4,
      members: ["Lleyne Loya", "Zaima Mahmood", "Daniel Esmond"]
    },
    {
      number: 5,
      members: ["Araceli Ortega Hernandez", "Teresa Ortega Hernandez", "Daniel Fox"]
    },
    {
      number: 6,
      members: ["Brenda Ruiz", "Jennifer Vielman", "Xavier Guzman Rivera"]
    },
    {
      number: 7,
      members: ["John Hinojosa", "Javier Orellana Mixco", "Luis Salazar", "Elijah Gutierrez"]
    },
    {
      number: 8,
      members: ["Brycen South", "Nikhil Sutter", "Luis Valenzuela", "John Velazquez"]
    }
  ],

  ratingScale: [
    { value: 1, label: "Little or weak effort", description: "Detrimental to team" },
    { value: 2, label: "Insufficient effort", description: "Met minimal standards of team" },
    { value: 3, label: "Sufficient effort", description: "Contributed adequately to team" },
    { value: 4, label: "Very strong work", description: "Contributed significantly to team" },
    { value: 5, label: "Excellent work", description: "Was crucial component to team's success" }
  ],

  emails: {
    // Team 1
    "Maria Arrieta Perez": "marrieta-perez@csub.edu",
    "Laura Carrillo": "lcarrillo1@csub.edu",
    "Eduardo Avalos": "eavalos10@csub.edu",
    // Team 2
    "Bhavika Damani": "bdamani@csub.edu",
    "Madison Guerrero": "mguerrero26@csub.edu",
    "Angel Avina": "aavina1@csub.edu",
    // Team 3
    "Yvette Jackson": "ywafford@csub.edu",
    "Prabhjot Kaur": "pkaur55@csub.edu",
    "Juan Chavez": "jchavez16@csub.edu",
    // Team 4
    "Lleyne Loya": "lloya1@csub.edu",
    "Zaima Mahmood": "zmahmood1@csub.edu",
    "Daniel Esmond": "desmond@csub.edu",
    // Team 5
    "Araceli Ortega Hernandez": "aortega-hernandez@csub.edu",
    "Teresa Ortega Hernandez": "tortega-hernandez@csub.edu",
    "Daniel Fox": "dfox7@csub.edu",
    // Team 6
    "Brenda Ruiz": "bruiz27@csub.edu",
    "Jennifer Vielman": "jvielman@csub.edu",
    "Xavier Guzman Rivera": "xguzman-rivera@csub.edu",
    // Team 7
    "John Hinojosa": "jhinojosa10@csub.edu",
    "Javier Orellana Mixco": "jorellana@csub.edu",
    "Luis Salazar": "lsalazar9@csub.edu",
    "Elijah Gutierrez": "Egutierrez72@csub.edu",
    // Team 8
    "Brycen South": "bsouth@csub.edu",
    "Nikhil Sutter": "nsutter@csub.edu",
    "Luis Valenzuela": "lvalenzueela@csub.edu",
    "John Velazquez": "jvelazquez7@csub.edu"
  }
};
